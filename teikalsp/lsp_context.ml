...
let update_text_documents context f =
  ...
  let value, text_documents = f text_documents in
  context.text_documents <- text_documents;
  value

let find_document context uri =
  match Document_uri_map.find_opt uri context.text_documents with
...
let change_text_document context uri cb =
  ...
  (text_document, Document_uri_map.add uri text_document text_documents)
