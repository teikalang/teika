...
module Ptree = struct
  ...
  let rec pp_term prec fmt term =
    ...
    | ( (PT_var _ | PT_hole _ | PT_string _ | PT_var_level _),
...
module Perror = struct
  ...
  let rec pp_error fmt error =
    ...
    | PE_literals_not_implemented -> fprintf fmt "literals not implemented"
...
and ptree_of_typed_pat ctx pat =
  let open Ptree in
  let (TP_with_type { pat; type_ }) = pat in
...
and perror_of_error ctx error =
  ...
  | TError_loc { error; loc } ->
      ...
      in
      loop loc error
  | TError_subtype_var_clash -> PE_var_clash
  | TError_subtype_type_clash -> PE_type_clash
  | TError_typer_unknown_var { name } -> PE_unknown_var { name }
  | TError_typer_not_a_forall { type_ } ->
      let type_ = ptree_of_term ctx type_ in
      PE_not_a_forall { type_ }
  | TError_typer_extensions_not_implemented -> PE_extensions_not_implemented
  | TError_typer_literals_not_implemented -> PE_literals_not_implemented
